...
static void test_custom_body_claude(void) {
    ...
    RJson *v = rjson_parse(a2, (const char *)out.data, out.len, &err);
    ASSERT_NOT_NULL(v);
    const RJson *ex = rjson_obj_get(v, "extra");
    ASSERT_NOT_NULL(ex);
    buf_free(&out);
    arena_destroy(a);
    arena_destroy(a2);
...
static void test_custom_body_google(void) {
    Arena *a = arena_create(0);
    const RikkaMessage *msgs[1];
    msgs[0] = mk_msg(a, RIKKA_ROLE_USER, "hi");
...
static void test_custom_headers_json(void) {
    /* custom_headers 是 HTTP 层消费(JSON 对象), 这里验证 JSON 合法 */
    Arena *a = arena_create(0);
    size_t err = 0;
    const char *hj = "{\"X-Test\":\"abc\",\"X-Num\":\"42\"}";
    RJson *v = rjson_parse(a, hj, strlen(hj), &err);
    ASSERT_NOT_NULL(v);
    ASSERT(v->type == RJSON_OBJECT);
    const RJson *xt = rjson_obj_get(v, "X-Test");
    ASSERT_NOT_NULL(xt);
    ASSERT(xt->type == RJSON_STRING);
    arena_destroy(a);
}
...
TEST(provider_sampling_params) {
    ...
    RJson *v = rjson_parse(a2, (const char *)out.data, out.len, &err);
    ASSERT_NOT_NULL(v);
    const RJson *t = rjson_obj_get(v, "temperature");
    ASSERT_NOT_NULL(t);
    ASSERT(t->u.number > 0.29 && t->u.number < 0.31);
    const RJson *tp = rjson_obj_get(v, "top_p");
    ASSERT_NOT_NULL(tp);
    ASSERT(tp->u.number > 0.94 && tp->u.number < 0.96);
    const RJson *to = rjson_obj_get(v, "translation_options");
    ASSERT_NOT_NULL(to);
    const RJson *sl = rjson_obj_get(to, "source_lang");
    ASSERT_NOT_NULL(sl);
    ASSERT(strcmp(sl->u.str.ptr, "auto") == 0);
    buf_free(&out);
    arena_destroy(a);
    arena_destroy(a2);
}
...
int run_provider_suite(void) {
    const RikkaTest tests[] = {
        ...
        RIKKA_TEST_REGISTER(provider, provider_sampling_params),
